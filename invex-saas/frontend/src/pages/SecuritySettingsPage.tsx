...
import {
  ...
  ClipboardList, ToggleLeft, ToggleRight, Bell
} from 'lucide-react';
...
export const SecuritySettingsPage: React.FC = () => {
  ...
  useEffect(() => {
    fetch(`${API}/security/audit-log/${USER_ID}`)
      .then(r => r.json())
  ...
  useEffect(() => {
    fetch(`${API}/security/device/check/${USER_ID}`, { method: 'POST' })
      .then(r => r.json())
      .then(setDeviceInfo)
      .catch(() => {});
  }, []);
  ...
  const handleExport = async () => {
    ...
    try {
      const res = await fetch(`${API}/compliance/export/${USER_ID}`);
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`${API}/compliance/delete/${USER_ID}`, {
        method: 'POST',
  ...
  const handleConsentToggle = async (type: string, value: boolean) => {
    ...
    setConsent(updated);
    try {
      await fetch(`${API}/compliance/consent/${USER_ID}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
