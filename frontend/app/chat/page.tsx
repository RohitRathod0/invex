...
import {
    ...
    AlertTriangle, Zap, BarChart2, Clock, ArrowUp, Plus, MessageSquare, Trash2
} from 'lucide-react';
    ...
    return (
        <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', color: '#fff' }}>
            ...
            <div style={{ width: '260px', flexShrink: 0, borderRight: '1px solid rgba(255,255,255,0.07)', display: 'flex', flexDirection: 'column', background: '#0D0D0D' }}>
                ...
                {/* Header + New Chat */}
                <div style={{ padding: '16px 14px 12px', flexShrink: 0, borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    <button onClick={startNewChat} style={{ width: '100%', background: 'rgba(200,241,53,0.08)', border: '1px solid rgba(200,241,53,0.18)', borderRadius: '10px', padding: '10px 14px', color: '#C8F135', cursor: 'pointer', fontSize: '13px', fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '7px', transition: 'all 0.15s' }}>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '8px 8px 0' }}>
                    {grouped.length === 0 && (
                        <div style={{ textAlign: 'center', padding: '32px 16px' }}>
                            <MessageSquare size={22} color="#2D3748" style={{ margin: '0 auto 8px' }} />
                            <p style={{ fontSize: '12px', color: '#374151' }}>No conversations yet</p>
                            <p style={{ fontSize: '11px', color: '#2D3748', marginTop: '4px' }}>Start a new chat above</p>
                        </div>
                    )}
                    {grouped.map(group => (
                        <div key={group.label} style={{ marginBottom: '16px' }}>
                            <p style={{ fontSize: '10px', color: '#3D4756', textTransform: 'uppercase', letterSpacing: '0.12em', padding: '4px 8px 6px', fontWeight: 700 }}>{group.label}</p>
                            {group.items.map(s => {
                                const isActive = s.id === activeSessionId;
                                const isPendingDelete = deletingId === s.id;
                                return (
                                    <div key={s.id}
                                        onClick={() => !isPendingDelete && switchSession(s.id)}
                                        style={{
                                            ...
                                            border: isPendingDelete
                                                ...
                                                : isActive ? '1px solid rgba(255,255,255,0.09)' : '1px solid transparent',
                                            marginBottom: '2px',
                                            transition: 'all 0.18s',
                                        }}
                                    >
                                        {isPendingDelete ? (
                                            /* ── Soft-delete confirm row ── */
                                            <>
                                                <span style={{ flex: 1, fontSize: '11px', color: '#EF4444', fontWeight: 500 }}>Delete this chat?</span>
                                                <button
                                                    onClick={(e) => confirmDelete(s.id, e)}
                                                    style={{ background: '#EF4444', border: 'none', borderRadius: '6px', padding: '3px 8px', color: '#fff', fontSize: '11px', fontWeight: 600, cursor: 'pointer', flexShrink: 0 }}
                                                >Yes</button>
                                                <button
                                                    onClick={cancelDelete}
                                                    style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '6px', padding: '3px 8px', color: '#9CA3AF', fontSize: '11px', cursor: 'pointer', flexShrink: 0 }}
                                                >No</button>
                                            </>
                                        ) : (
                                            /* ── Normal row ── */
                                            <>
                                                <MessageSquare size={13} color={isActive ? '#C8F135' : '#4B5563'} style={{ flexShrink: 0 }} />
                                                <span style={{ flex: 1, fontSize: '12px', color: isActive ? '#E5E7EB' : '#6B7280', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.title}</span>
                                                <button
                                                    onClick={(e) => requestDelete(s.id, e)}
                                                    title="Delete chat"
                                                    style={{ background: 'none', border: 'none', color: '#374151', cursor: 'pointer', padding: '3px', flexShrink: 0, display: 'flex', alignItems: 'center', borderRadius: '5px', transition: 'all 0.15s' }}
                                                >
                                                    <Trash2 size={11} />
                                                </button>
                                            </>
                ...
                {/* Bottom bar — session count */}
                <div style={{ padding: '10px 16px 14px', borderTop: '1px solid rgba(255,255,255,0.05)', flexShrink: 0 }}>
                    <p style={{ fontSize: '10px', color: '#2D3748', textAlign: 'center' }}>
                        {sessions.length} conversation{sessions.length !== 1 ? 's' : ''} · synced to server
                    </p>
                </div>
            </div>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
                ...
                <div style={{ borderTop: '1px solid rgba(255,255,255,0.07)', padding: '16px 24px', background: '#0A0A0A', flexShrink: 0 }}>
                    ...
                    <p style={{ fontSize: '10px', color: '#2D3748', textAlign: 'center', marginTop: '8px' }}>
                        Press Enter to send · Shift+Enter for new line · Chats saved to server
                    </p>
                </div>
            </div>
